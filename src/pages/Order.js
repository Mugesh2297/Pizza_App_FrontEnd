...
function Order() {
  ...
  return (
    <>
      <h1 className='text-center text-white  mt-5 mb-5'>My Orders</h1>
      {loading && (<Loader />)}
      {error && (<Error error="Something went wrong" />)}
      {
        orders && orders.map((order) => (
          <Container className="container border p-4 orderComponent mb-2">
            <Row>
              <Col md={4}>
              ...
              <Col md={4}>
                ...
                {
                  order.isDelivered === true ? <h6 className='text-success'>Delivered</h6> : <h6 className='text-warning'>Estimate Time : 30 Mint</h6>
                }
              </Col>
            </Row>
          </Container>
